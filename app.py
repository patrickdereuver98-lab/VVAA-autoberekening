...
if kenteken_input:
    ...
    if auto:
        ...
        if is_full_ev:
            ...
            start_60_dt = datetime.datetime(start_jr, start_mnd, 1)
            is_vervallen_ev = datetime.datetime.now() >= (start_60_dt + relativedelta(months=60))
            if is_vervallen_ev: peil_jaar = (start_60_dt + relativedelta(months=60)).year
        ...
        with st.container(border=True):
            ...
            with top3:
                ...
                if totaal_km > 0 and (z_km / totaal_km) < 0.10:
                    st.warning(f"⚠️ **LET OP:** De auto wordt voor slechts {(z_km / totaal_km)*100:.1f}% zakelijk gebruikt. Minimaal 10% zakelijk gebruik is vereist.")
                if is_young_manual and not is_young_auto:
                    st.warning(f"⚠️ **Let op:** Voertuig is pas {leeftijd.years} jaar oud (eis: 15 jaar).")
                elif is_young_auto and not is_young_manual:
                    st.info(f"💡 **Tip:** Voertuig is ouder dan 15 jaar. De youngtimer-regeling is waarschijnlijk voordeliger.")
                if is_vervallen_ev and not is_young_manual:
                    st.info(f"⚡ **Let op:** De 60-maandenregel EV is verlopen. Regels peiljaar {peil_jaar} toegepast.")
            ...
            with col1:
                ...
                if is_minder_dan_500: calc_bijt = 0.0
                    ...
                    if "€" in gekozen_bijt:
                        ...
                        calc_bijt = (min(auto['catalogusprijs'], cap) * perc) + (max(0, auto['catalogusprijs'] - cap) * 0.22)
                    else: calc_bijt = auto['catalogusprijs'] * perc
                bijt_bruto = st.number_input("Bijtelling (€/jr)", value=float(round(calc_bijt)))
            with col2:
            ...
            with col3:
                ...
                if schat:
                    with st.expander("ℹ️ Uitleg schatting"):
                        st.write("- Onderhoud: € 0,04 per km.\n- Overig: € 250,- p/j.\n- Verzekering: blijft op € 0.")
                oh = round(totaal_km * 0.04) if schat else 0
                onderhoud = st.number_input("Onderhoud (€/j)", value=float(oh))
                verzekering = st.number_input("Verzekering (€/j)", value=0.0)
                overige = st.number_input("Overig (€/j)", value=250.0 if schat else 0.0)
                lease_k, rente_k = 0.0, 0.0
                if is_geleased:
                    st.markdown("#### Financiering")
                    st.caption("ℹ️ *Let op: bij Operational Lease zijn zaken als wegenbelasting, onderhoud en verzekering vaak al inbegrepen. Zet die velden hierboven dan op 0.*")
                    lease_k = st.number_input("Leasekosten (€/j)", value=0.0); rente_k = st.number_input("Rentekosten (€/j)", value=0.0)
        ...
        with st.container(border=True):
            ...
            if bijt_bruto > tot_k and not is_minder_dan_500:
                st.warning(f"⚖️ **LET OP:** De bijtelling is hoger dan de autokosten en daarom afgetopt op de werkelijke kosten (€ {fmt(bijt_def)}).")
            ...
            with res2:
                ...
                st.write(f"Vergoeding: **€ 0,23 p/km** | Zakelijk: **{fmt(z_km)} km**")
        ...
        if gevalideerd:
            ...
            pdf = VVAAPDF(); pdf.set_auto_page_break(auto=False); pdf.add_page(); f = "Arial"
            ...
            pdf.cell(35, 6, " < 500 km prive:", fill=True); pdf.cell(155, 6, "Ja" if is_minder_dan_500 else "Nee", fill=True, ln=True)
            ...
            lbl_bijt = "Bijtelling (afgetopt):" if bijt_bruto > tot_k else "Bijtelling:"
            pdf.cell(45, 6, lbl_bijt); pdf.cell(45, 6, f"- EUR {fmt(bijt_def)}", align='R', ln=True); pdf.ln(2); pdf.set_fill_color(245)
    ...
    else: st.error("❌ Geen voertuig gevonden.")
