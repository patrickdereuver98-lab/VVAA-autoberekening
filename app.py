...
<style>
    ...
    /* DROPDOWN FIX: ALTIJD WITTE ACHTERGROND */
        ...
        with st.container(border=True):
            ...
            with col2:
                ...
                if is_bs or not is_ev:
                    ...
                    prijs_l = st.number_input("Prijs per Liter (€)", value=ap["diesel"] if "diesel" in bs_l else ap["benzine"])
                    brandstof_kosten = st.number_input("Brandstofkosten p/j (€)", value=float(round(((z_km+p_km)/100)*verb_l*prijs_l)))
                if is_ev:
                    verb_kwh = st.number_input("Verbruik Stroom (kWh/100km)", value=0.0)
                    laad_kosten = st.number_input("Laadkosten p/j (€)", value=float(round(((z_km+p_km)/100)*verb_kwh*0.40)))
            ...
            with col3:
                ...
                onderhoud = st.number_input("Onderhoud (€/j)", value=float(round(totaal_km * 0.04)) if gebruik_schatting else 0.0)
                verzekering = st.number_input("Verzekering (€/j)", value=0.0)
                overige = st.number_input("Overige kosten (€/j)", value=250.0 if gebruik_schatting else 0.0)
        ...
        if gevalideerd:
            ...
            class VVAAPDF(FPDF):
                ...
                def footer(self):
                    ...
                    self.set_text_color(255); self.set_font(self.font_fam, '', 9); self.cell(0, 15, clean("VvAA | Voor zorgverleners, door zorgverleners"), align='C', ln=True)
            ...
            pdf.cell(35, 5, "Relatie:"); pdf.cell(70, 5, clean(klant_naam)); pdf.cell(35, 5, "Lidnr:"); pdf.cell(70, 5, clean(klant_nummer), ln=True); pdf.ln(3)
            
            # HERSTEL: VOLLEDIGE VOERTUIGSPECIFICATIES
            ...
            pdf.cell(35, 6, " Toelating:", fill=True); pdf.cell(155, 6, f"{toelating_nl} ({leeftijd.years} jaar, {leeftijd.months} mnd)", fill=True, ln=True)
            ...
            pnt = ["- Getoonde cijfers zijn afgeronde hele euro's.", "- Wegenbelasting is gebaseerd op tarieven 2026.", "- Berekening is een schatting op basis van uw opgave."]
